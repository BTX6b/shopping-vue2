  ...
  <div class="clearfix selector">
    ...
    <div class="type-wrap" v-for="item in attrsList" :key="item.attrId">
      ...
      <div class="fl value">
        <ul class="type-list">
          <li
            v-for="(value, index) in item.attrValueList"
            :key="index"
            @click="attrInfo(item.attrName, value)"
          >
            <a>{{ value }}</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    tradMarkInfo(item) {
      console.log(item)
      this.$emit('getTradMarkInfo', item)
    },
    // 参数信息
    attrInfo(attrName, attrValueList) {
      // console.log(attrName, attrValueList)
      this.$emit('getAttrInfo', attrName, attrValueList)
    }
  }
}
</script>
